...
The sticky desktop selection bar keeps the exact selected count and combined storage size, per-page choices, a destructive review action, and select-all across every filtered page in one place. Selecting all filtered results disables individual checkboxes to make the selection scope unambiguous and reports the combined size across the full filtered result set. Protection and retention do not silently defeat an explicit manual selection; identity, active playback, tracker, shared-torrent, and freshness checks still fail closed immediately before mutation.
