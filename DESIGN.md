...
Manual Management uses a filter-first workbench: Overseerr user, target Arr instance, tracker conditions, and meaningful-watch state form a compact header before any result or selection control. Result and series-summary rows hold four aligned columns—selection, media identity, request context, and tracker state—through 780px; below that breakpoint they become two-column records with metadata stacked under the identity and no horizontal overflow. Its desktop selection bar stays sticky at the top of the workspace, then becomes static below the compact-shell breakpoint and single-column on phones.
...
Manual Management is a requester-owned, cross-instance operational workbench reached from primary navigation. Four explicit filters—Overseerr user, target Radarr or Sonarr instance, tracker conditions, and meaningful-watch state—precede the result set. The watch filter operates on the stored meaningful-watch timestamp, with explicit All, Meaningfully watched, and Never meaningfully watched states. Adjacent sort controls order titles by name, last meaningful watch, Overseerr request date, release date, or combined size in either direction; unknown dates remain at the end. Movie requests render as direct compact rows. TV requests render as native disclosure rows whose expanded children are independently selectable seasons; season zero is always written as **Specials**, while the series itself remains in Sonarr.

The sticky desktop selection bar keeps the exact selected count and combined storage size, per-page choices, a destructive review action, and select-all across every filtered page in one place. Selecting all filtered results visibly checks the current page while preserving editable item checkboxes; unchecked exceptions persist across pagination and are subtracted from both the total count and combined size. Protection and retention do not silently defeat an explicit manual selection; identity, active playback, tracker, shared-torrent, and freshness checks still fail closed immediately before mutation.
...
Each service row combines a 2:3 media tile, identity and URL, stacked text states, routine actions, and a progressively disclosed configuration region. Read-only enablement, management mode, and Active Management are visibly separate concepts. Protected mode blocks automated proposals while still permitting explicitly confirmed Manual Management actions when Active Management is enabled; Ignored disables destructive actions entirely. Active Management remains locked until its prerequisites are met, and enabling it requires consequence copy plus an explicit confirmation when available.
